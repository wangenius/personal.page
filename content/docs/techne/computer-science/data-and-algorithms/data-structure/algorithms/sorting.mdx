...
![排序比较](/img/compared_sorts.jpeg)

![外部排序](/img/外部排序.png)

12. 置换选择排序：用于*生成初始归并段*。根据buffer大小初始化归并段。每所有记录都要进行两次`I/O`操作。

13. 最佳归并树BMT：使用huffman树的构造方法来构造BMT，用于减少归并次数。
...
![](/img/selection-sort-animate.svg)
...
![](/img/insertion-sort-animate.svg)

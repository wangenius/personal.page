...
export function useMDXComponents(components: MDXComponents): MDXComponents {
  return {
    ...
    h1: (props) => createElement('h1', { 
      ...
      id: props.children?.toString().toLowerCase().replace(/[^a-z0-9\u4e00-\u9fa5]+/g, '-').replace(/^-+|-+$/g, '')
    }),
    h2: (props) => createElement('h2', { 
      ...props,
      className: "text-3xl font-semibold text-gray-900 dark:text-gray-50 mb-5 mt-8",
      id: props.children?.toString().toLowerCase().replace(/[^a-z0-9\u4e00-\u9fa5]+/g, '-').replace(/^-+|-+$/g, '')
    }),
    h3: (props) => createElement('h3', { 
      ...props,
      className: "text-2xl font-semibold text-gray-900 dark:text-gray-50 mb-4 mt-6",
      id: props.children?.toString().toLowerCase().replace(/[^a-z0-9\u4e00-\u9fa5]+/g, '-').replace(/^-+|-+$/g, '')
    }),
    h4: (props) => createElement('h4', { 
      ...props,
      className: "text-xl font-semibold text-gray-900 dark:text-gray-50 mb-4 mt-6",
      id: props.children?.toString().toLowerCase().replace(/[^a-z0-9\u4e00-\u9fa5]+/g, '-').replace(/^-+|-+$/g, '')
    }),
    h5: (props) => createElement('h5', { 
      ...props,
      className: "text-lg font-semibold text-gray-900 dark:text-gray-50 mb-3 mt-5",
      id: props.children?.toString().toLowerCase().replace(/[^a-z0-9\u4e00-\u9fa5]+/g, '-').replace(/^-+|-+$/g, '')
    }),
    h6: (props) => createElement('h6', { 
      ...props,
      className: "text-base font-semibold text-gray-900 dark:text-gray-50 mb-3 mt-5",
      id: props.children?.toString().toLowerCase().replace(/[^a-z0-9\u4e00-\u9fa5]+/g, '-').replace(/^-+|-+$/g, '')
    }),
    ...
    pre: ({ children }) => {
      ...
      const language = childProps.className?.replace(/language-/, '') || 'plaintext';
    ...
    // 引用块样式优化 - 减小上下边距
    blockquote: ({ children }) => (
      <MDXBlockQuote>{children}</MDXBlockQuote>
    ),

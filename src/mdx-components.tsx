...
export function useMDXComponents(components: MDXComponents): MDXComponents {
  return {
    ...
    h2: ({ children }) => (
      <h2 className="text-2xl font-bold mb-3 mt-8 text-gray-900 dark:text-gray-100">{children}</h2>
    ),
    h3: ({ children }) => (
      <h3 className="text-xl font-bold mb-2 mt-6 text-gray-900 dark:text-gray-100">{children}</h3>
    ),
    
    // 段落和文本组件
    p: ({ children }) => (
      <p className="mb-6 leading-7 text-gray-700 dark:text-gray-300">{children}</p>
    ),
    strong: ({ children }) => (
    ...
    // 列表组件
    ul: ({ children }) => (
      <ul className="list-disc list-inside mb-6 ml-6 space-y-2 text-gray-700 dark:text-gray-300">{children}</ul>
    ),
    ol: ({ children }) => (
      <ol className="list-decimal list-inside mb-6 ml-6 space-y-2 text-gray-700 dark:text-gray-300">{children}</ol>
    ),
